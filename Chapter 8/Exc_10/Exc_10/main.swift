class OrderedQuadraticHashing: CustomStringConvertible {
    ...
    func doubleHashFunction(_ start: Int) -> Int        {
        return start % 2 == 0 ? 1 : 2 // по определению, должно работать =)
    }
    ...
    func insert(_ key : Int, _ value: String){
        if count == array.count{
            ...
            var newPair = HashPair(key, value)
            while let caret = array[index]{
                if caret.value > newPair.value{
                    array[index] = newPair
                    newPair = caret
                }
    ...
    func find(_ key : Int) -> String?{
        ...
        while let item = array[index], item.key != key, iteration != array.count - 1 {
            index = (index + (step * step)) % array.count
            iteration += 1
            step += 1
        }
...
var hash = OrderedQuadraticHashing()
hash.makeItems()
print(hash)
