...
class OrderedTree {
    ...
    func addNode(_ newValue: String){
    ...
    private func addNode(_ root : BinaryNode, _ newValue: String){
    ...
    func findNode(_ target: String) -> BinaryNode?{
        return findNode(self.root, target)
    }
    
    private func findNode(_ root : BinaryNode?, _ target: String) -> BinaryNode?{
        if let root = root{
    ...
    func removeNode(_ target: String){
        removeNode(nil, self.root, target)
    }
    
    private func removeNode(_ parent : BinaryNode?, _ viewed : BinaryNode?, _ target: String){
    ...
    var folderStructure : String{
        get{
            if let root = root{
                var result = ""
                var indent = ""
                printPretty(root, &result, &indent, true)
                return result
            }
    ...
    fileprivate func printPretty(_ leaf: BinaryNode, _ result : inout String, _ indent : inout String, _ last : Bool)
    {
        result += indent
        if (last)
        {
        ...
        var kinders : [BinaryNode] = [];
        if let left = leaf.leftChild{
            kinders.append(left)
        }
        if let right = leaf.rightChild{
            kinders.append(right)
        }
        for i in 0..<kinders.count{
            var temp_indent = indent
            printPretty(kinders[i] , &result, &temp_indent, i == kinders.count - 1);
        }
    }
